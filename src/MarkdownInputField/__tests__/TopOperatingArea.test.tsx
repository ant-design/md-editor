...
describe('TopOperatingArea - BackTo按钮功能', () => {
  ...
  it('BackTo按钮存在且可访问', () => {
    ...
    expect(topButton).toHaveTextContent('Back to Top');
    expect(bottomButton).toHaveTextContent('Back to Bottom');
  });
});
...
describe('TopOperatingArea - 自定义操作按钮', () => {
  ...
  it('自定义按钮应该支持点击事件', () => {
    ...
    const operationBtnRender = () => (
      <button data-testid="clickable-btn" onClick={handleClick} type="button">
        可点击按钮
      </button>
    );
  ...
  it('按钮容器应该有正确的样式类名', () => {
    const operationBtnRender = () => (
      <button data-testid="styled-btn" type="button">
        样式按钮
      </button>
    );
...
describe('TopOperatingArea - 组合功能测试', () => {
  it('应该同时支持自定义按钮和BackTo按钮', () => {
    const operationBtnRender = () => (
      <button data-testid="combo-btn" type="button">
        组合按钮
      </button>
    );
  ...
  it('应该支持完整的props组合', () => {
    const targetRef = createRef<HTMLDivElement>();
    const operationBtnRender = () => (
      <button data-testid="full-combo-btn" type="button">
        全功能按钮
      </button>
    );
    ...
    expect(screen.getByTestId('back-to-bottom')).toHaveAttribute(
      'data-target',
      'custom',
    );
    expect(
