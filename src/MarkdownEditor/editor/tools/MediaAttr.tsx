...
export const MediaAttr = observer(() => {
  ...
  const del = useCallback(() => {
    ...
    if (pre) {
      ...
      Transforms.select(
        store.editor,
        Editor.start(store.editor, Path.parent(nodeRef.current![1])),
      );
    }
    ...
    if (isHotkey('enter', e)) {
      ...
      if (node) {
        ...
        if (Editor.hasPath(store.editor, nextPath)) {
          ...
          Transforms.select(
            store.editor,
            Editor.start(store.editor, nextLinePath),
          );
        } else {
  ...
  return (
    <div
      ...
      z-10 absolute dark:text-gray-300 text-gray-500 h-8 text-sm text-gray-200/70 px-2 select-none flex items-center ${
    ...
    >
      ...
      <div
        className={
          'rounded-sm dark:hover:bg-gray-200/10 hover:bg-gray-100 duration-200 ml-2 px-1 py-0.5'
        }
      >
        <CheckOutlined
          onClick={setUrl}
      ...
      <div
        className={
          'w-[1px] h-5 dark:bg-gray-200/10 bg-gray-200 flex-shrink-0 mx-1'
        }
      ></div>
      <div
        className={
          'rounded-sm dark:hover:bg-gray-200/10 hover:bg-gray-100 duration-200 px-1 py-0.5'
        }
      >
        <ReloadOutlined
