...
describe('EnterKey', () => {
  ...
  describe('run()', () => {
    ...
    it('should handle card-before element', () => {
      const mockSelection = { anchor: { path: [0, 0], offset: 0 }, focus: { path: [0, 0], offset: 0 } };
      editor.selection = mockSelection;
      (Range.isCollapsed as Mock).mockReturnValue(true);
      
      const cardBeforeElement = { type: 'card-before', children: [{ text: '' }] };
      const path = [0];
  ...
  describe('head() method', () => {
    it('should insert paragraph at start of heading', () => {
      const headElement: HeadNode = { type: 'head', level: 1, children: [{ text: 'Heading' }] };
      const path = [0];
      const sel = {
        anchor: { path: [0, 0], offset: 0 },
  ...
  describe('empty() method', () => {
    it('should handle empty blockquote removal', () => {
      ...
      expect(mockEvent.preventDefault).toHaveBeenCalled();
      expect(Transforms.delete).toHaveBeenCalledWith(editor, { at: blockquoteParent[1] });
      expect(Transforms.insertNodes).toHaveBeenCalledWith(
        editor,
        { type: 'paragraph', children: [{ text: '' }] },
    ...
    it('should handle empty list-item when it is the only item', () => {
      const path = [0, 0];
      const listItemParent = [{ type: 'list-item', children: [{ text: '' }] }, [0]];
      ...
      expect(mockEvent.preventDefault).toHaveBeenCalled();
      expect(Transforms.delete).toHaveBeenCalledWith(editor, { at: listParent[1] });
      expect(Transforms.insertNodes).toHaveBeenCalledWith(
        editor,
        EditorUtils.p,
  ...
  describe('table() method', () => {
    it('should insert break with Ctrl+Shift+Enter', () => {
      ...
      const sel = { anchor: { path: [0, 0, 0], offset: 0 }, focus: { path: [0, 0, 0], offset: 0 } };
      const mockEvent = { preventDefault: vi.fn(), metaKey: true, shiftKey: true } as any;
    ...
    it('should insert new table row with Ctrl+Enter', () => {
      const tableNode = [{ type: 'table', children: [] }, [0, 0]] as any;
      const sel = { anchor: { path: [0, 0, 0], offset: 0 }, focus: { path: [0, 0, 0], offset: 0 } };
      const mockEvent = { preventDefault: vi.fn(), metaKey: true, shiftKey: false } as any;
    ...
    it('should navigate to next row on Enter', () => {
      ...
      const sel = { anchor: { path: [0, 0, 1], offset: 0 }, focus: { path: [0, 0, 1], offset: 0 } };
      ...
      expect(Transforms.select).toHaveBeenCalledWith(
        editor,
        { path: [1, 1], offset: 0 }
      );
    });
  ...
  describe('paragraph() method', () => {
    it('should return undefined when no matching pattern found', () => {
      const paragraphElement = { type: 'paragraph', children: [{ text: 'Regular text' }] };
      const path = [0, 0];
      const sel = {
    ...
    it('should handle paragraph in list-item with Cmd/Ctrl key', () => {
      const paragraphElement = { type: 'paragraph', children: [{ text: 'List item text' }] };
      const path = [0, 0];
      const sel = {
      ...
      const mockParent = [{ type: 'list-item', children: [paragraphElement] }, [0]];
      (Editor.parent as Mock).mockReturnValue(mockParent);
    ...
    it('should handle new list-item creation in checked list', () => {
      const paragraphElement = { type: 'paragraph', children: [{ text: 'Checked item' }] };
      const path = [0, 0];
      const sel = {
        anchor: { path: [0, 0], offset: 12 },
        focus: { path: [0, 0], offset: 12 }
      ...
      const mockParent = [{ type: 'list-item', children: [paragraphElement], checked: true }, [0]];
      (Editor.parent as Mock).mockReturnValue(mockParent);
      (Editor.end as Mock).mockReturnValue({ path: [0, 0], offset: 12 });
      (Point.equals as Mock).mockReturnValue(true);
      (Path.hasPrevious as Mock).mockReturnValue(false);
  ...
  describe('empty() method - extended cases', () => {
    ...
    it('should handle list-item with non-empty content', () => {
      ...
      const listItemParent = [
        { type: 'list-item', children: [{ text: 'Some content' }, { text: 'more' }] }, 
        [0]
      ];
      const listParent = [{ type: 'list', children: [listItemParent[0]] }, []];
  ...
  describe('table() method - extended cases', () => {
    it('should navigate to next element after table when no next row exists', () => {
      ...
      (Path.parent as Mock).mockReturnValueOnce([0]).mockReturnValueOnce([0, 0]);
      (Path.next as Mock).mockReturnValueOnce([1]).mockReturnValueOnce([1]);
      (Editor.hasPath as Mock).mockReturnValueOnce(false).mockReturnValueOnce(true);
      (Editor.start as Mock).mockReturnValue({ path: [1], offset: 0 });
  ...
  describe('head() method - extended cases', () => {
    it('should split heading text when cursor is in middle', () => {
      const headElement: HeadNode = { type: 'head', level: 2, children: [{ text: 'Split Heading' }] };
      const path = [0];
      const sel = {
        anchor: { path: [0, 0], offset: 5 },
        focus: { path: [0, 0], offset: 5 }
