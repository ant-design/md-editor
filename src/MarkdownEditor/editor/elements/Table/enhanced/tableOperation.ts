...
import { createRandomId, ELTYPE } from './utils';
import { SEL_CELLS } from './weak-maps';
...
const insertRow = (editor: Editor, selection: any) => {
  ...
  if (cellsPath.length !== 0) {
    ...
    if (sel.anchor && sel.focus) {
      maxPos =
        sel.focus.path[2] >= sel.anchor.path[2]
          ? sel.focus.path
          : sel.anchor.path;
    } else {
      return;
  ...
  const curRowRowspanNodes = Editor.nodes(editor, {
    ...
    match: (n: any) => {
      return (
        typeof n.colspan === 'number' &&
        typeof n.rowspan === 'number' &&
        !(n.colspan === 0 && n.rowspan === 0)
      );
    },
  });
  let next;
  ...
  for (let i = 0; i < col; i++) {
    ...
    const tmpColspan =
      typeof child[i].colspan === 'number'
        ? child[i].colspan === 1
          ? null
          : child[i].colspan
        : null;
    const tmpRowspan =
      typeof child[i].rowspan === 'number' &&
      typeof child[i].colspan === 'number'
        ? Number.parseInt(child[i].rowspan) *
            Number.parseInt(child[i].colspan) >
          0
          ? 1
          : child[i].colspan > 1
            ? 1
  ...
  console.log(
    ...
    { hwEach: newHwEach, row: newRowNum },
    { at: [maxPos[0], 1] },
  );

  Transforms.setNodes(
    editor,
    JSON.parse(JSON.stringify({ hwEach: newHwEach, row: newRowNum })),
    {
      at: [maxPos[0], 1],
    },
  );
...
const insertRowReverse = (editor: Editor, selection: any) => {
  ...
  const curRowNum =
    newHwEach.length <= minPos[2] + 1 ? newHwEach.length - 1 : minPos[2] + 1;
...
const deleteRow = (editor: any, selection: any) => {
    ...
    if (sel.anchor && sel.focus) {
      maxPos =
        sel.focus.path[2] >= sel.anchor.path[2]
          ...
          : sel.anchor.path;
      minPos =
        sel.focus.path[3] <= sel.anchor.path[3]
          ? sel.focus.path
          : sel.anchor.path;
    } else {
      return;
  ...
  if (maxPos[2] === row - 1 && minPos[2] === 0) {
    ...
    Transforms.setNodes(
      editor,
      { hwEach: newHwEach, row: newRowNum } as Partial<Node>,
      { at: [index[0], 1] },
    );
  }
};
...
const insertCol = (editor: any, selection: any) => {
  ...
  for (let i = 1; i <= row; i++) {
    ...
    console.log(
      '{prevNode, isLastCount, tmpNode.colspan}',
      prevNode,
      isLastCount,
      tmpNode.colspan,
    );
    ...
    const tmpIsNotReal =
      typeof tmpNode.colspan !== 'undefined' ||
      tmpPath[3] + tmpNode.colspan - 1 > prevPath[3];
    const node: any = {
      type: ELTYPE.TABLE_CELL,
      key: createRandomId(),

      colspan: isLastColspan ? null : tmpIsNotReal ? 0 : null,
      rowspan: isLastColspan
        ? null
        : typeof prevNode.rowspan === 'number'
          ? prevNode.rowspan
          : null,
      children: [
        {
  ...
  pointsLeftTop.forEach(([node, path]) => {
    for (let i = 0; i < node.rowspan; i++) {
      Transforms.setNodes(
        editor,
        JSON.parse(JSON.stringify({ colspan: (node.colspan as number) + 1 })),
        {
  ...
  Transforms.setNodes(
    editor,
    JSON.parse(JSON.stringify({ hwEach: newHwEach, column: newColNum })),
    { at: [initialPath[0], 1] },
  );
};
...
const deleteCol = (editor: any, selection: any) => {
  ...
  Transforms.setNodes(
    editor,
    { hwEach: newHwEach, column: newColNum } as Partial<Node>,
    { at: [left[0], 1] },
  );
};
...
const mergeCell = (editor: any, selection: any) => {
  ...
  for (let row = pointLeftTop[2] + rowspan - 1; row >= pointLeftTop[2]; row--) {
    for (
      let col = pointLeftTop[3] + colspan - 1;
      col >= pointLeftTop[3];
      col--
    ) {
  ...
  for (let i = start.path[2]; i < end.path[2] + 1; i++) {
    ...
    for (let j = start.path[3]; j < end.path[3] + 1; j++) {
      const cellContent: any = (
        Editor.node(editor, [start.path[0], 1, i, j])[0] as any
      ).children;
      if (
        !(i === start.path[2] && j === start.path[3]) &&
        !(cellContent.length === 1 && cellContent[0]?.children[0]?.text === '')
      ) {
  ...
  for (let i = 0; i < y_len; i++) {
    for (let j = 0; j < x_len; j++) {
      if (i === 0 && j === 0) {
        ...
        arr[i][j].rowspan = rowspan;
        arr[i][j].colspan = null;
      } else if (j === 0) {
        arr[i][j].rowspan = null;
        arr[i][j].colspan = colspan;
      } else {
        arr[i][j].rowspan = null;
        arr[i][j].colspan = null;
...
const unmergeCell = (editor: any, selection: any) => {
  ...
  const cells = Editor.nodes(editor, {
    ...
    match: (n: any) =>
      ...
      (typeof (n as any).colspan !== 'undefined' ||
        typeof (n as any).rowspan !== 'undefined'),
  });
