...
describe('AttachmentButtonPopover', () => {
  describe('SupportedFileFormats', () => {
    ...
    it('should have correct file extensions for images', () => {
      const imageFormat = SupportedFileFormats.find(format => format.type === '图片');
    ...
    it('should have correct file extensions for documents', () => {
      const docFormat = SupportedFileFormats.find(format => format.type === '文档');
    ...
    it('should have appropriate max sizes', () => {
      const imageFormat = SupportedFileFormats.find(format => format.type === '图片');
      const videoFormat = SupportedFileFormats.find(format => format.type === '视频');
  ...
  describe('AttachmentSupportedFormatsContent', () => {
    it('should render supported formats content', () => {
      render(<AttachmentSupportedFormatsContent supportedFormats={SupportedFileFormats} />);
      
      expect(screen.getByText('支持上传的文件类型和格式：')).toBeInTheDocument();
    ...
    it('should render file size limits', () => {
      ...
      expect(screen.getByText('单个最大 10.00 KB')).toBeInTheDocument();
      expect(screen.getByText('单个最大 50.00 KB')).toBeInTheDocument();
      expect(screen.getByText('单个最大 100.00 KB')).toBeInTheDocument();
    });
    ...
    it('should render custom content when provided', () => {
      ...
      render(<AttachmentSupportedFormatsContent supportedFormats={customFormats} />);
      
      expect(screen.getByText('Custom content for this format')).toBeInTheDocument();
    });

    it('should return null when no supported formats provided', () => {
      const { container } = render(<AttachmentSupportedFormatsContent supportedFormats={[]} />);
  ...
  describe('Edge Cases', () => {
    it('should handle empty format extensions array', () => {
      ...
      render(<AttachmentSupportedFormatsContent supportedFormats={formatsWithEmptyExtensions} />);
    ...
    it('should handle formats with zero max size', () => {
      ...
      render(<AttachmentSupportedFormatsContent supportedFormats={formatsWithZeroSize} />);
      
      expect(screen.getByText('单个最大 0.00 KB')).toBeInTheDocument();
    });
    ...
    it('should handle formats with very large max size', () => {
      ...
      render(<AttachmentSupportedFormatsContent supportedFormats={formatsWithLargeSize} />);
      
      expect(screen.getByText('单个最大 1.00 GB')).toBeInTheDocument();
    });
    ...
    it('should handle single extension', () => {
      ...
      render(<AttachmentSupportedFormatsContent supportedFormats={singleExtensionFormat} />);
