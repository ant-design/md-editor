...
vi.mock('../../src/MarkdownEditor', () => ({
  BaseMarkdownEditor: React.forwardRef((props: any, ref: any) => {
    const [content, setContent] = React.useState(props.value || props.initValue || '');
...
describe('MarkdownInputField 断言测试', () => {
  ...
  describe('基础属性断言', () => {
    ...
    it('应该正确应用 className', () => {
      ...
      const { container } = render(<MarkdownInputField className={className} />);
    ...
    it('应该正确处理附件上传', async () => {
      render(
        <MarkdownInputField
          attachment={{
            enable: true,
            upload: vi.fn().mockResolvedValue({ url: 'http://example.com/file.txt' }),
          }}
        />
      );
  ...
  describe('属性验证断言', () => {
    ...
    it('应该正确处理样式相关的 props', () => {
      const styleProps = {
        bgColorList: ['#fff', '#f0f0f0', '#e0e0e0', '#d0d0d0'] as [string, string, string, string],
        borderRadius: 8,
        style: { minHeight: '100px' },
        className: 'custom-class',
      };

      render(<MarkdownInputField {...styleProps} />);
  ...
  describe('功能完整性断言', () => {
    ...
    it('应该支持自定义操作按钮渲染', () => {
      const customActions = (props: any, defaultActions: React.ReactNode[]) => [
        ...defaultActions,
        <button key="custom" type="button" data-testid="custom-action">Custom</button>
      ];
