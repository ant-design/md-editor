...
describe('Welcome 组件', () => {
  ...
  it('应该只渲染标题当没有描述时', () => {
    const { container } = render(<Welcome title="只有标题" />);
  ...
  it('应该支持自定义根节点样式类名', () => {
    const { container } = render(
      <Welcome title="根节点类名" rootClassName="custom-root-class" />,
    );
  ...
  it('应该支持自定义样式', () => {
    ...
    const { container } = render(
      <Welcome title="自定义样式" style={customStyle} />,
    );
  ...
  it('应该支持 React 节点作为标题', () => {
    ...
    render(<Welcome title={titleNode} />);
  ...
  it('应该支持 React 节点作为描述', () => {
    ...
    render(<Welcome title="标题" description={descriptionNode} />);
  ...
  it('应该支持数字作为标题', () => {
    render(<Welcome title={123} />);

    expect(screen.getByText('123')).toBeInTheDocument();
  });

  it('应该支持数字作为描述', () => {
    render(<Welcome title="标题" description={456} />);

    expect(screen.getByText('456')).toBeInTheDocument();
  });

  it('应该支持布尔值作为标题', () => {
    const { container } = render(<Welcome title={true} />);
  ...
  it('应该支持数组作为标题', () => {
    render(<Welcome title={['数组', '标题']} />);

    expect(screen.getByText('数组标题')).toBeInTheDocument();
  });

  it('应该支持数组作为描述', () => {
    render(<Welcome title="标题" description={['数组', '描述']} />);
