...
const useCreateComponentPlugin = (
  tagName: string,
  targetType: string,
    ...
    toMarkdown: [
      {
        ...
        convert: (node: any) => {
          return `<${tagName} context='${node.context}'></${tagName}>`;
        },
      },
    ],
    elements: {
...
export default () => {
  const mdTagPlugin = useCreateComponentPlugin(
    'md-tag',
    'customTag',
  ...
  return (
    <BaseMarkdownEditor
      initValue="123<md-tag context='时间'></md-tag>789"
