...
| onProgress | 进度回调函数，接收当前进度 (0-1) 作为参数，0-0.5 为解析，0.5-1 为插入 | (progress: number) => void | -      |
...
当 `useRAF=true` 且分块数量 > 10 时，`setMDContent` 会分两个阶段异步处理：

1. **解析阶段**（0-50% 进度）：使用 RAF 分批解析 markdown chunks，每帧解析 `batchSize/10` 个分块
2. **插入阶段**（50-100% 进度）：使用 RAF 分批插入节点到编辑器，每帧插入 `batchSize` 个节点

这样可以避免大文档解析时卡住主线程，保持页面响应。
